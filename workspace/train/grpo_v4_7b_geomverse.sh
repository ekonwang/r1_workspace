...
# In this version training, we append the auxiliary reward to the reward function portforlio.
# Which checks if the completion's thinking process contains auxiliary lines.
...
python3 -m openrlhf.cli.train_ppo_ray \
  ...
  --init_kl_coef 0.01 \
  --prompt_data .temp/datasets/GeomVerse/TRAIN/TRAIN_MIX/grpo_conversations_aux.jsonl \
  --input_key 'prompt' \
  --label_key 'answer' \
  --apply_chat_template \
